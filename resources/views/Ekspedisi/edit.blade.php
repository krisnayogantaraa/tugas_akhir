...
@section('content')
<p class="text-5xl mb-3 text-gray-900 dark:text-white">Selesaikan Exspedisi</p>

<form @if(auth()->user()->type == "admin")
    action="{{ route('ekspedisi.update', $Ekspedisi->id) }}"
    @else
    action="{{ route('ekspedisi2.update', $Ekspedisi->id) }}"
    @endif
    ...
    <div class="grid gap-6 mb-6 md:grid-cols-2">
        <div>
            ...
            <input type="text" id="first_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ $Ekspedisi->no_keputusan_pengadilan }}" name="no_keputusan_pengadilan">
        </div>
    </div>
    <div class="mb-6">
